...
"""
Implementation from https://github.com/AdelaideAuto-IDLab/Attend-And-Discriminate
"""
...
def plot_confusion(
    ...
    if(not len(cm) == len(class_map)): #class map does not fit to labels
        class_map = None
    #Only use the labels that appear in the data
    if not class_map:
        class_map = [str(idx) for idx in range(len(np.unique(y_true)))]
    if normalize:
    ...
    ax.set(
        xticks=np.arange(len(class_map)),
        yticks=np.arange(len(class_map)),
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(path_save, "cm_" + str(epoch) + ".pdf"), bbox_inches="tight"
    )
    # plt.show()
    plt.close()
